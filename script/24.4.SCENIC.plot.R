...
plot_pyscenic <- function(inloom='aucell.loom',incolor=incolor,inrss='seurat_annotations_rss.rds',inrds='subset.rds',infun='median', ct.col='seurat_annotations',inregulons=NULL,ingrn='grn.scRNA.tsv',ntop1=5,ntop2=50){
  ## test
  if(F){
  inloom='aucell.loom'
  inrss='subtype_rss.rds'
  inrds=file.path(dir_in, "sce_Hep.rds")
  infun='median'
  ct.col='subtype'
  inregulons=NULL
  ingrn='grn.scRNA.tsv'
  ...
  print(pheatmap(pn2,scale=scale,show_rownames = T, main='plotRSS unique regulons'))
  dev.off()
  ...
  print(pheatmap(rss,scale=scale,show_rownames = T,main='all regulons RSS'))
  dev.off()
...
plot_pyscenic(inloom='aucell.loom',
  ...
  infun='median', 
  ct.col='subtype',
  inregulons=NULL,ingrn='grn.scRNA.tsv',ntop1=5,ntop2=50)
