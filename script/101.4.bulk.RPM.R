...
################################################################################
##### read data
df_exp <- read.table(file.path(dir_in, "3.Expression/all_samples_expression_RPM.xls"), header = TRUE, sep = "\t")
...
df_bulk <- df_bulk[rowSums(df_bulk[, -1]) != 0, ] 
...
saveRDS(df_bulk, file=file.path(dir_dataset, 'df_RPM.rds'))
